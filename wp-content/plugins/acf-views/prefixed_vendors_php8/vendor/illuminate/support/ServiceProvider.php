...
use Closure;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Console\Application as Artisan;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Contracts\Foundation\CachesConfiguration;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Contracts\Foundation\CachesRoutes;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Contracts\Support\DeferrableProvider;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Database\Eloquent\Factory as ModelFactory;
...
abstract class ServiceProvider
{
    ...
    public static function addProviderToBootstrapFile(string $provider, ?string $path = null)
    {
        ...
        $providers = collect(require $path)->merge([$provider])->unique()->sort()->values()->map(fn($p) => '    ' . $p . '::class,')->implode(\PHP_EOL);
        $content = '<?php
