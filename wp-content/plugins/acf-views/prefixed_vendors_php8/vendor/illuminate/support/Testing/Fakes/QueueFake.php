...
use Closure;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Contracts\Queue\Queue;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Queue\CallQueuedClosure;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Queue\QueueManager;
...
class QueueFake extends QueueManager implements Fake, Queue
{
    ...
    public function pushed($job, $callback = null)
    {
        ...
        return collect($this->jobs[$job])->filter(fn($data) => $callback($data['job'], $data['queue'], $data['data']))->pluck('job');
    }
    /**
    ...
    {
        return collect($this->jobs)->flatten(1)->filter(fn($job) => $job['queue'] === $queue)->count();
    }
    /**
