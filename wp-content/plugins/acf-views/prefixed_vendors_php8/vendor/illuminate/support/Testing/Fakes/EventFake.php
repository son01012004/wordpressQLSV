...
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Contracts\Events\Dispatcher;
...
class EventFake implements Dispatcher, Fake
{
    ...
    public function assertNothingDispatched()
    {
        ...
        $eventNames = collect($this->events)->map(fn($events, $eventName) => \sprintf('%s dispatched %s %s', $eventName, \count($events), Str::plural('time', \count($events))))->join("\n- ");
    ...
    public function dispatched($event, $callback = null)
    {
        ...
        return collect($this->events[$event])->filter(fn($arguments) => $callback(...$arguments));
    }
    /**
    ...
    protected function shouldDispatchEvent($eventName, $payload)
    {
        ...
        return collect($this->eventsToDispatch)->filter(function ($event) use($eventName, $payload) {
            return $event instanceof Closure ? $event($eventName, $payload) : $event === $eventName;
        })->isNotEmpty();
    }
    /**
