...
namespace Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Support\Testing\Fakes;

use Closure;
use Exception;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Contracts\Notifications\Dispatcher as NotificationDispatcher;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Contracts\Notifications\Factory as NotificationFactory;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Contracts\Queue\ShouldQueue;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Contracts\Translation\HasLocalePreference;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Notifications\AnonymousNotifiable;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Support\Collection;
...
class NotificationFake implements Fake, NotificationDispatcher, NotificationFactory
{
    ...
    {
        $notificationNames = collect($this->notifications)->map(fn($notifiableModels) => collect($notifiableModels)->map(fn($notifiables) => collect($notifiables)->keys()))->flatten()->join("\n- ");
    ...
    {
        $actualCount = collect($this->notifications)->flatten(1)->reduce(fn($count, $sent) => $count + \count($sent[$notification] ?? []), 0);
