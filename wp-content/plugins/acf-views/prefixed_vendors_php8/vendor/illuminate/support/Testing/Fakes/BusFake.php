...
use Closure;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Bus\BatchRepository;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Bus\ChainedBatch;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Bus\PendingBatch;
use Org\Wplake\Advanced_Views\Optional_Vendors\Illuminate\Contracts\Bus\QueueingDispatcher;
...
class BusFake implements Fake, QueueingDispatcher
{
    ...
    {
        $jobNames = collect($this->batches)->map(fn($batch) => $batch->jobs->map(fn($job) => \get_class($job)))->flatten()->join("\n- ");
    ...
    {
        return collect($this->jobsToDispatch)->filter(function ($job) use($command) {
            return $job instanceof Closure ? $job($command) : $job === \get_class($command);
        })->isNotEmpty();
    }
    /**
