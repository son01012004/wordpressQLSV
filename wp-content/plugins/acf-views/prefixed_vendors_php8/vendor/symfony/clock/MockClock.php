...
final class MockClock implements ClockInterface
{
    ...
    public function __construct(\DateTimeImmutable|string $now = 'now', \DateTimeZone|string|null $timezone = null)
    {
        if (\PHP_VERSION_ID >= 80300 && \is_string($timezone)) {
            ...
            try {
                ...
                throw new \Org\Wplake\Advanced_Views\Optional_Vendors\DateInvalidTimeZoneException($e->getMessage(), $e->getCode(), $e);
            }
        }
        if (\is_string($now)) {
    ...
    public function modify(string $modifier) : void
    {
        if (\PHP_VERSION_ID < 80300) {
            $this->now = @$this->now->modify($modifier) ?: throw new \Org\Wplake\Advanced_Views\Optional_Vendors\DateMalformedStringException(\error_get_last()['message'] ?? \sprintf('Invalid modifier: "%s". Could not modify MockClock.', $modifier));
            return;
        }
