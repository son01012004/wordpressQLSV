...
wp_enqueue_script( 'rishi-companion-tab' );

// Define the expected parameters.
$expected_params = array(
	'postsTabRecentLabel'   => __( 'Recent Posts', 'rishi-companion' ),
	'postsTabPopularLabel'  => __( 'Popular Posts', 'rishi-companion' ),
	'postsTabCount'         => 3,
...
list( $recent_tab_label, $popular_tab_label, $number_of_posts, $show_thumbnail, $tab_image_size, $show_date ) = rishi_companion_list( array_keys( $expected_params ), array_merge( $expected_params, $attributes ) );
...
$image_size = ( 'default' === $tab_image_size ) ? 'thumbnail' : 'full';
?>
	...
	<div class="posts-tab-content">
		...
		if ( isset( $popular_posts[0] ) ) :
			foreach ( $popular_posts as $popular_post ) :
				$post_thumbnail = get_post_thumbnail_id( $popular_post->ID );
				...
				<div class="tab-content">
					...
					<a href="<?php echo esc_url( $post_url ); ?>" rel="noopener" class="post-thumbnail <?php echo ! $post_thumbnail ? esc_attr( 'fallback-img' ) : ''; ?>">
						<?php
						if ( $post_thumbnail ) :
							echo wp_get_attachment_image( $post_thumbnail, $image_size );
						endif;
						?>
					</a>
					<?php endif; ?>
