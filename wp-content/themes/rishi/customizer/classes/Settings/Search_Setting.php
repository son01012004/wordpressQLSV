...
class Search_Setting extends Customize_Settings {

	protected function add_settings() {

		$search_defaults = self::get_search_default_value();

		$this->add_setting('search_title_panel', array(
			'label'         => __( 'Page Title', 'rishi' ),
			'control'          => ControlTypes::PANEL,
			'innerControls' => array(
				\Rishi\Customizer\Helpers\Basic::uniqid( 'rishi_search_general' ) => array(
					'title'   => __( 'General', 'rishi' ),
					'control'    => ControlTypes::TAB,
					'options' => array(
						'ed_search_header'       => array(
							'label'   => __( 'Enable Page Header', 'rishi' ),
							'control' => ControlTypes::INPUT_SWITCH,
							'value'   => 'yes',
						),
						'breadcrumbs_ed_search' => array(
							'label' => __( 'Breadcrumb', 'rishi' ),
							'control'  => ControlTypes::INPUT_SWITCH,
							'value' => $search_defaults['breadcrumbs_ed_search'],
							'divider' => 'top',
							'conditions' => [
								'ed_search_header' => 'yes'
							]
						),
						'search_page_label'     => array(
						...
						'search_page_search_margin' => array(
				...
				\Rishi\Customizer\Helpers\Basic::uniqid() => array(
					...
					'control'    => ControlTypes::TAB,
					'options' => array(
						'search_page_header_content_background' => array(
		...
		$this->add_setting('search_page_layout', array(
			'label'   => __( 'Search Page Layout', 'rishi' ),
			'help'    => __( 'Choose the search page layout for your site.', 'rishi' ),
			'control'    => ControlTypes::IMAGE_PICKER,
			'value'   => $search_defaults['search_page_layout'],
			'divider' => 'top',
		...
		$this->add_setting('search_post_structure', array(
		...
		$this->add_setting('search_post_navigation', array(
			'label'   => __( 'Posts Navigation', 'rishi' ),
			'control'    => ControlTypes::INPUT_SELECT,
			'value'   => $search_defaults['search_post_navigation'],
			'view'    => 'text',
			'design'  => 'block',
			'divider' => 'top',
			'choices' => \Rishi\Customizer\Helpers\Basic::ordered_keys(
		...
		$this->add_setting('search_sidebar_layout', array(
			'label'   => __( 'Sidebar Layout', 'rishi' ),
			'control'    => ControlTypes::IMAGE_PICKER,
			'value'   => $search_defaults['search_sidebar_layout'],
			'attr'    => array(
				'data-type'    => 'background',
				'data-usage'   => 'layout-style',
				'data-columns' => '2',
			),
			'help'    => __( 'Choose sidebar layout for search page.', 'rishi' ),
			'divider' => 'top',
			'choices' => array(
		...
		$this->add_setting('search_layout', array(
			'label'   => __( 'Search Container', 'rishi' ),
			'control'    => ControlTypes::INPUT_SELECT,
			'value'   => $search_defaults['search_layout'],
			'view'    => 'text',
			'design'  => 'block',
			'divider' => 'top',
			'choices' => \Rishi\Customizer\Helpers\Basic::ordered_keys(
			...
			'help'    => __( 'Choose search page container layout.', 'rishi' ),
		));

		$this->add_setting('search_layout_streched_ed', array(
			'label' => __( 'Stretch Layout', 'rishi' ),
			'help'  => __( 'This setting stretches the container width.', 'rishi' ),
			'control'  => ControlTypes::INPUT_SWITCH,
			'value' => $search_defaults['search_layout_streched_ed'],
			'divider' => 'top',
		));
	}

	/**
	 * Layout choices for the Pages
	 *
	 * @return void
	 */
	...
	/**
	 * Set default value for search page.
	 */
	protected static function get_search_default_value() {
